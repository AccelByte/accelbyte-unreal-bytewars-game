...
void ShowSettingsMessage(const FText& msg, const FText& title)
{
#if (ENGINE_MAJOR_VERSION == 5 && ENGINE_MINOR_VERSION >= 3) || ENGINE_MAJOR_VERSION > 5
    FMessageDialog::Open(EAppMsgType::Type::Ok, msg, title);
#else
    FMessageDialog::Open(EAppMsgType::Type::Ok, msg, &title);
#endif
}
...
void UBlackBoxSettings::ShowMustRestartDialog()
{
    ...
    ShowSettingsMessage(MustRestartMessage, MustRestartTitle);
}

void UBlackBoxSettings::ShowDialog(const FString& DialogTitle, const FString& DialogMessage)
{
    const FText Title = FText::FromString(DialogTitle);
    const FText Message = FText::FromString(DialogMessage);
    ShowSettingsMessage(Message, Title);
}
