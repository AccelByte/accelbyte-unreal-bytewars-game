...
private:
    ...
    int isIssueReporterHotkeyChaged = 0;
...
#endif
#if !WITH_EDITOR && 0 // currently hitch detection is disabled
        bbx_set_hitching_video_location(TCHAR_TO_UTF8(*ModuleInstance->IssueFolder));
#endif
    }
}
...
bool FAccelByteBlackBoxSDKModule::LoadSettings()
{
    ...
    if (FPaths::FileExists(BlackBoxConfig)) {
        ...
        if (SDKInfo.IsEnable) {
            ...
            if (GConfig->GetString(
                ...
                FInputChord LoadedInputChord = BlackBoxIssueReporter::DeserializeInputChord(IssueHotkeyStr);
                if (LoadedInputChord.IsValidChord() && !BlackBoxIssueReporter::IsChordUsedAsMappings(LoadedInputChord) &&
                    BlackBoxIssueReporter::GetKeyCodes(LoadedInputChord.Key)) {
                    SDKInfo.IssueReporterHotkey = LoadedInputChord;
                }
                else {
                    UE_LOG(LogBlackBox, Log, TEXT("Problem encountered with loaded hotkey, using default Issue Reporter Hotkey"));
                    SDKInfo.IssueReporterHotkey = GetDefault<UBlackBoxSettings>()->IssueReporterHotkey;
                }
            }
            else {
                SDKInfo.IssueReporterHotkey = GetDefault<UBlackBoxSettings>()->IssueReporterHotkey;
            ...
            if (!GConfig->GetBool(
                    TEXT("BlackBoxSettings"),
                    TEXT("IncludeGameLogInIssueReport"),
                    SDKInfo.IncludeGameLogInIssueReport,
                    BlackBoxConfig)) {
                SDKInfo.IncludeGameLogInIssueReport = GetDefault<UBlackBoxSettings>()->IncludeGameLogInIssueReport;
            }
        }
        else {
    ...
    if (SDKInfo.IsEnable) {
        if (!SDKInfo.IssueReporterHotkey.IsValidChord()) {
    ...
    UObject* ModifiedObject, FPropertyChangedEvent& PropertyChangedEvent)
{
    if (UBlackBoxSettings* settings = Cast<UBlackBoxSettings>(ModifiedObject)) {
        ...
        if (Info.IssueReporterHotkey != (settings->IssueReporterHotkey)) {
            if (!settings->IssueReporterHotkey.IsValidChord()) {
                const char* msg = "Issue Reporter hotkey inputted is not valid, using previous value...";
                UE_LOG(LogBlackBox, Warning, TEXT("%s"), UTF8_TO_TCHAR(msg));
                settings->ShowDialog("BlackBox Setting", msg);
                settings->IssueReporterHotkey = Info.IssueReporterHotkey;
            }
            else if (BlackBoxIssueReporter::IsChordUsedAsMappings(settings->IssueReporterHotkey)) {
                const char* msg =
                    "The hotkey for the Issue Reporter has been previously mapped. The previous value will "
                    "be retained.";
                UE_LOG(LogBlackBox, Warning, TEXT("%s"), UTF8_TO_TCHAR(msg));
                settings->ShowDialog("BlackBox Setting", msg);
                settings->IssueReporterHotkey = Info.IssueReporterHotkey;
            }
            else if (!BlackBoxIssueReporter::GetKeyCodes(settings->IssueReporterHotkey.Key)) {
                const char* msg = "The hotkey for the Issue Reporter isn't supported. The previous value will "
                                  "be retained.";
                UE_LOG(LogBlackBox, Warning, TEXT("%s"), UTF8_TO_TCHAR(msg));
                settings->ShowDialog("BlackBox Setting", msg);
                settings->IssueReporterHotkey = Info.IssueReporterHotkey;
                ...
                GConfig->SetString(
                    ...
                    *(BlackBoxIssueReporter::SerializeInputChord(settings->IssueReporterHotkey)),
                    BlackBoxConfig);
                Info.IssueReporterHotkey = settings->IssueReporterHotkey;
                isIssueReporterHotkeyChaged = 1;
            }
        }
        if (Info.IncludeGameLogInIssueReport != settings->IncludeGameLogInIssueReport) {
            GConfig->SetBool(
                TEXT("BlackBoxSettings"),
                TEXT("IncludeGameLogInIssueReport"),
                settings->IncludeGameLogInIssueReport,
                BlackBoxConfig);
            Info.IncludeGameLogInIssueReport = settings->IncludeGameLogInIssueReport;
        }
...
bool FAccelByteBlackBoxSDKModule::OnEngineTick(float TickTime)
{
    ...
    if (bbx_config_get_enable_issue_reporter() && !isIssueReporterHotkeyChaged) {
        FInputChord Hotkey = BlackBoxIssueReporter::GetIssueReporterHotkey();

        if (Hotkey.IsValidChord()) {
            uint32 VirtualKeyCode = 0;

            if (Hotkey.Key.IsValid()) {
                VirtualKeyCode = BlackBoxIssueReporter::GetKeyCodes(Hotkey.Key);
            }

            bool bCtrlReqFulfilled = Hotkey.bCtrl ? (GetAsyncKeyState(VK_CONTROL) & 0x8000) != 0 : true;
            bool bShiftReqFulfilled = Hotkey.bShift ? (GetAsyncKeyState(VK_SHIFT) & 0x8000) != 0 : true;
            bool bAltReqFulfilled = Hotkey.bAlt ? (GetAsyncKeyState(VK_MENU) & 0x8000) != 0 : true;

            if (bCtrlReqFulfilled && bShiftReqFulfilled && bAltReqFulfilled) {
                short trigger_state_now = GetKeyState(static_cast<int>(VirtualKeyCode)) & 0x8000;
            ...
            UE_LOG(
                LogBlackBox,
                Warning,
                TEXT("Failed to get Issue Reporter Hotkey/The Hotkey is invalid, please set a valid hotkey and "
                     "re-enable Issue Reporter "
                     "afterwards."));
            bbx_config_set_enable_issue_reporter(false);
        }
    }
    isIssueReporterHotkeyChaged =
        isIssueReporterHotkeyChaged ? isIssueReporterHotkeyChaged > 100 ? 0 : isIssueReporterHotkeyChaged + 1 : 0;
#endif
    return true;
}
...
void FAccelByteBlackBoxSDKModule::EnableHitchDetection(bool bShouldEnable)
{
#if !WITH_EDITOR && 0 // currently hitch detection is disabled
	if (bShouldEnable)
	{
		...
		Stats.NewFrameDelegate.AddRaw(this, &FAccelByteBlackBoxSDKModule::CheckHitches);
	}
	else
	{
#if ENGINE_MAJOR_VERSION > 5 || (ENGINE_MAJOR_VERSION == 5 && ENGINE_MINOR_VERSION > 0)
...
void FAccelByteBlackBoxSDKModule::CheckHitches(int64 Frame)
{
#if !WITH_EDITOR && 0 // currently hitch detection is disabled
    //  when synced, this time will be the full time of the frame, whereas the above don't include any waits
    const FStatsThreadState& Stats = FStatsThreadState::GetLocalState();
    const float GameThreadTimeWithWaits =
        (float)FPlatformTime::ToMilliseconds64(Stats.GetFastThreadFrameTime(Frame, EThreadType::Game));
    const float RenderThreadTimeWithWaits =
        (float)FPlatformTime::ToMilliseconds64(Stats.GetFastThreadFrameTime(Frame, EThreadType::Renderer));
    const float FullFrameTime = FMath::Max(GameThreadTimeWithWaits, RenderThreadTimeWithWaits);

    FFrameRate MinimumFrameRate = FFrameRate(10, 1); // 10 FPS

    // check for hitch (if application not backgrounded)
    const float TimeThreshold = MinimumFrameRate.AsInterval() * 1000.0f;
    if (FullFrameTime > TimeThreshold) {
        ...
        float HitchedFPS = MinimumFrameRate.AsDecimal();
        ...
        UE_LOG(
            LogBlackBox,
            Log,
            TEXT("Hitch detected: FullFrameTime=%f, GameThreadTimeWithWaits=%f, RenderThreadTimeWithWaits=%f, "
                 "Threshold=%f, GameThreadTime=%f, RenderThreadTime=%f"),
            FullFrameTime,
            GameThreadTimeWithWaits,
            RenderThreadTimeWithWaits,
            TimeThreshold,
            GameThreadTime,
            RenderThreadTime);
        bbx_set_is_hitching(true);
    }
    else {
        bbx_set_is_hitching(false);
    }
#endif
}
