...
FOnlineIdentityAccelBytePtr UManagingChatSubsystem::GetIdentityInterface()
{
    ...
    if (!ensure(Subsystem))
    {
        UE_LOG_MANAGINGCHAT(Warning, TEXT("The online subsystem is invalid. Please make sure OnlineSubsystemAccelByte is enabled and DefaultPlatformService under [OnlineSubsystem] in the Engine.ini set to AccelByte."));
        return nullptr;
    }
