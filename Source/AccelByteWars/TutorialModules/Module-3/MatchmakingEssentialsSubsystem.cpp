...
void UMatchmakingEssentialsSubsystem::Deinitialize()
{
	...
	AAccelByteWarsGameModeBase::OnGetTeamIdFromSessionDelegate.Unbind();
	AAccelByteWarsGameSession::OnRegisterServerDelegate.Unbind();
	AAccelByteWarsGameSession::OnUnregisterServerDelegate.Unbind();
	UMatchLobbyWidget::OnGenerateOnlineTeamEntries.Unbind();
	UMatchLobbyWidget::OnQuitLobbyDelegate.Unbind();
	UPauseWidget::OnQuitGameDelegate.Unbind();
...
void UMatchmakingEssentialsSubsystem::StartMatchmaking(APlayerController* PC, const FString& MatchPool, const FOnMatchmakingDelegate& OnMatchmaking)
{
	...
	// Cache matchmaking information. It will be used to retry matchmaking.
	AssociatedPC = PC;
	LastMatchPool = MatchPool;
	OnMatchmakingHandle = OnMatchmaking;

	// If the player is already in a session, then destroy it first.
	if (SessionInterface->GetNamedSession(NAME_GameSession))
	{
		const FOnDestroySessionCompleteDelegate OnDestroyToRematchmakingCompleteDelegate = FOnDestroySessionCompleteDelegate::CreateUObject(this, &UMatchmakingEssentialsSubsystem::OnDestroyToRematchmakingComplete);
		SessionInterface->DestroySession(NAME_GameSession, OnDestroyToRematchmakingCompleteDelegate);
		return;
	}

	const FUniqueNetIdPtr LocalPlayerId = GetAssociatedPlayerUniqueId();
	...
	MatchmakingCompleteDelegateHandle = SessionInterface->AddOnMatchmakingCompleteDelegate_Handle(FOnMatchmakingCompleteDelegate::CreateUObject(this, &UMatchmakingEssentialsSubsystem::OnMatchmakingComplete));

	// Bind on-start matchmaking delegate and then start the matchmaking process.
	const FOnStartMatchmakingComplete OnStartMatchmakingCompleteDelegate = FOnStartMatchmakingComplete::CreateUObject(this, &UMatchmakingEssentialsSubsystem::OnStartMatchmakingComplete);
...
void UMatchmakingEssentialsSubsystem::CancelMatchmaking(APlayerController* PC)
{
	...
	CancelMatchmakingCompleteDelegateHandle = SessionInterface->AddOnCancelMatchmakingCompleteDelegate_Handle(FOnCancelMatchmakingCompleteDelegate::CreateUObject(this, &UMatchmakingEssentialsSubsystem::OnCancelMatchmakingComplete));
...
void UMatchmakingEssentialsSubsystem::JoinSession(const FOnlineSessionSearchResult& Session)
{
	if (!ensure(SessionInterface.IsValid()))
	{
	...
	JoinSessionCompleteDelegateHandle = SessionInterface->AddOnJoinSessionCompleteDelegate_Handle(FOnJoinSessionCompleteDelegate::CreateUObject(this, &UMatchmakingEssentialsSubsystem::OnJoinSessionComplete));
...
void UMatchmakingEssentialsSubsystem::OnMatchmakingComplete(FName SessionName, bool bWasSuccessful)
{
	...
	JoinSession(CurrentMatchmakingSearchHandle->SearchResults[0]);
	CurrentMatchmakingSearchHandle.Reset();
}
...
void UMatchmakingEssentialsSubsystem::OnCancelMatchmakingComplete(FName SessionName, bool bWasSuccessful)
{
	...
	if (bWasSuccessful) 
	{
		...
		OnMatchmakingHandle.ExecuteIfBound(EMatchmakingState::None);
	}
	else 
	{
...
void UMatchmakingEssentialsSubsystem::OnJoinSessionComplete(FName SessionName, EOnJoinSessionCompleteResult::Type Result)
{
	if (!IsGameSessionValid(SessionName))
	{
		OnMatchmakingHandle.ExecuteIfBound(EMatchmakingState::FindMatchFailed);
	...
	else
	{
		...
		SessionServerUpdateDelegateHandle = SessionInterface->AddOnSessionServerUpdateDelegate_Handle(FOnSessionServerUpdateDelegate::CreateUObject(this, &UMatchmakingEssentialsSubsystem::OnSessionServerUpdate));
	}
}

void UMatchmakingEssentialsSubsystem::OnSessionServerUpdate(FName SessionName)
{
	if (!IsGameSessionValid(SessionName)) 
	{
		return;
	...
	TravelClient(SessionName);
}

void UMatchmakingEssentialsSubsystem::OnDestroyToRematchmakingComplete(FName SessionName, bool bWasSuccessful)
{
	if (IsGameSessionValid(SessionName) && bWasSuccessful)
	{
		// Retry matchmaking.
		ensure(AssociatedPC);
		StartMatchmaking(AssociatedPC, LastMatchPool, OnMatchmakingHandle);
	}
	else 
	{
		OnMatchmakingHandle.ExecuteIfBound(EMatchmakingState::FindMatchFailed);
...
void UMatchmakingEssentialsSubsystem::GenerateOnlineTeamEntries(TArray<FUniqueNetIdPtr> PlayerUniqueNetIds, TArray<UPlayerEntryWidget*> PlayerEntryWidgets)
{
	// Get Players' AccelByte Id
	TArray<FString> PlayerIds;
	for (FUniqueNetIdPtr RawNetId : PlayerUniqueNetIds) 
	{
		...
		{
			UE_LOG(LogTemp, Warning, TEXT("Player's AccelByte Net Id us not valid. Aborting to generate online team entries."));
			return;
		}
	
		PlayerIds.Add(PlayerNetId->GetAccelByteId());
	}

	// Get Player's Information
	FRegistry::User.BulkGetUserInfo(
		PlayerIds,
		THandler<FListBulkUserInfo>::CreateWeakLambda(this, [PlayerEntryWidgets](const FListBulkUserInfo& Result)
		{
			int32 EntryIndex = 0;
			for (FBaseUserInfo UserInfo : Result.Data)
			{
				// Set username based on player's account.
				UPlayerEntryWidget* PlayerEntry = PlayerEntryWidgets[EntryIndex];
				if (!UserInfo.Username.IsEmpty())
				{
					PlayerEntry->SetUsername(FText::FromString(UserInfo.Username));
				}

				FString AvatarUrl = UserInfo.AvatarUrl;
				FString AvatarId = FBase64::Encode(AvatarUrl);

				// Try to set avatar image from cache.
				FCacheBrush CacheAvatarBrush = UUtilitiesSubsystem::GetImageFromCache(AvatarId);
				if (CacheAvatarBrush.IsValid())
				{
					// Set Image From Cache Brush
					PlayerEntry->SetAvatarTint(FLinearColor::White);
					PlayerEntry->SetAvatar(*CacheAvatarBrush.Get());
					continue;
				}

				// Get avatar image from URL if it is not exists in cache.
				if (!AvatarUrl.IsEmpty())
				{
					UUtilitiesSubsystem::GetImageFromURL(
						AvatarUrl,
						AvatarId,
						FOnImageReceived::CreateLambda([UserInfo, PlayerEntry](const FCacheBrush ImageResult)
						{
							// Set image brush here.
							PlayerEntry->SetAvatarTint(FLinearColor::White);
		...
		{
			UE_LOG_MATCHMAKING_ESSENTIALS(Warning, TEXT("%d: %s"), ErrorCode, *ErrorMessage);
		})
	);
}

int32 UMatchmakingEssentialsSubsystem::GetTeamIdFromSession(APlayerController* PC)
{
	...
	// Cannot find team assignment, return none.
	return INDEX_NONE;
}

void UMatchmakingEssentialsSubsystem::TravelClient(FName SessionName)
{
	if (!IsGameSessionValid(SessionName))
	{
		return;
	...
	if (SessionInterface->GetResolvedConnectString(SessionName, TravelUrl) && !TravelUrl.IsEmpty())
	{
		...
		AssociatedPC->ClientTravel(TravelUrl, TRAVEL_Absolute);
	}
	else
	{
...
FUniqueNetIdPtr UMatchmakingEssentialsSubsystem::GetAssociatedPlayerUniqueId() const
{
	if (!ensure(AssociatedPC)) 
	{
		return nullptr;
	...
	ULocalPlayer* LocalPlayer = AssociatedPC->GetLocalPlayer();
	if (!ensure(LocalPlayer))
	{
		return nullptr;
	}
