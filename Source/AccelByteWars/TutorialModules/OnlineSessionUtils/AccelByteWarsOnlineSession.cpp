...
#pragma region Playing With Party
    ...
    GetABSessionInt()->OnCancelMatchmakingCompleteDelegates.AddUObject(this, &ThisClass::OnCancelPartyMatchmakingComplete);
    GetABSessionInt()->OnMatchmakingExpiredDelegates.AddUObject(this, &ThisClass::OnPartyMatchmakingExpired);

    GetSessionInt()->OnCreateSessionCompleteDelegates.AddUObject(this, &ThisClass::OnCreatePartyMatchComplete);
...
#pragma region Playing With Party
    ...
    GetABSessionInt()->OnCancelMatchmakingCompleteDelegates.RemoveAll(this);
    GetABSessionInt()->OnMatchmakingExpiredDelegates.RemoveAll(this);

    GetSessionInt()->OnCreateSessionCompleteDelegates.RemoveAll(this);
    GetABSessionInt()->OnJoinSessionCompleteDelegates.RemoveAll(this);
    GetABSessionInt()->OnV2SessionInviteReceivedDelegates.RemoveAll(this);
#pragma endregion
}
...
#pragma region "Playing With Party"
void UAccelByteWarsOnlineSession::OnStartPartyMatchmakingComplete()
{
    // Abort if not a party match.
    if (!GetABSessionInt()->IsInPartySession() ||
        GetPartyMembers().Num() <= 1)
    {
        return;
    }
    ...
    if (IsPartyLeader(UserId))
    {
        return;
    }

    UE_LOG_ONLINESESSION(Log, TEXT("Party matchmaking started by party leader."));
...
void UAccelByteWarsOnlineSession::OnPartyMatchmakingComplete(FName SessionName, bool bSucceeded)
{
    // Abort if not a party match.
    if (!GetPredefinedSessionNameFromType(EAccelByteV2SessionType::GameSession).IsEqual(SessionName) ||
        !GetABSessionInt()->IsInPartySession() ||
        GetPartyMembers().Num() <= 1)
    {
        return;
    }
    ...
    if (bSucceeded)
    {
        UE_LOG_ONLINESESSION(Log, TEXT("Party matchmaking found. Currently joining the match."));
    ...
    {
        UE_LOG_ONLINESESSION(Warning, TEXT("Party matchmaking is failed."));

        if (GetPromptSubystem())
        {
            GetPromptSubystem()->HideLoading();
            GetPromptSubystem()->PushNotification(
                FText::FromString("Party Matchmaking is Failed"),
                FString(""));
        }
    }
}

void UAccelByteWarsOnlineSession::OnCancelPartyMatchmakingComplete(FName SessionName, bool bSucceeded)
{
    // Abort if not a party match.
    if (!GetPredefinedSessionNameFromType(EAccelByteV2SessionType::GameSession).IsEqual(SessionName) ||
        !GetABSessionInt()->IsInPartySession() || 
    ...
    if (bSucceeded)
    {
        UE_LOG_ONLINESESSION(Log, TEXT("Party Matchmaking is canceled by party leader."));
...
void UAccelByteWarsOnlineSession::OnPartyMatchmakingExpired()
{
    // Abort if not a party match.
    if (!GetABSessionInt()->IsInPartySession() ||
        GetPartyMembers().Num() <= 1)
    ...
    UE_LOG_ONLINESESSION(Warning, TEXT("Party matchmaking expired."));
...
void UAccelByteWarsOnlineSession::OnCreatePartyMatchComplete(FName SessionName, bool bSucceeded)
{
    // Abort if not a party match.
    if (!GetPredefinedSessionNameFromType(EAccelByteV2SessionType::GameSession).IsEqual(SessionName) ||
        !GetABSessionInt()->IsInPartySession() ||
    ...
    for (auto& Member : GetPartyMembers())
    {
        ...
        {
            UE_LOG_ONLINESESSION(Log, TEXT("Send party match invitation to: %s."), *MemberABId->GetAccelByteId());
            GetABSessionInt()->SendSessionInviteToFriend(UserId.ToSharedRef().Get(), SessionName, Member.Get());
        }
    }
}

void UAccelByteWarsOnlineSession::OnJoinPartyMatchComplete(FName SessionName, EOnJoinSessionCompleteResult::Type Result)
{
    // Abort if not a party match.
    if (!GetPredefinedSessionNameFromType(EAccelByteV2SessionType::GameSession).IsEqual(SessionName) ||
        !GetABSessionInt()->IsInPartySession() ||
    ...
    if (Result != EOnJoinSessionCompleteResult::Type::Success)
    {
        UE_LOG_ONLINESESSION(Warning, TEXT("Failed to join party match."));
...
void UAccelByteWarsOnlineSession::OnPartyMatchInviteReceived(const FUniqueNetId& UserId, const FUniqueNetId& FromId, const FOnlineSessionInviteAccelByte& Invite)
{
    ...
    if (IsPartyLeader(UserId.AsShared())) 
    {
        return;
    }
    ...
    if (!PC)
    {
        UE_LOG_ONLINESESSION(Warning, TEXT("Cannot join a party match invitation from party leader. PlayerController is not valid."));
        return;
    }

    const int32 LocalUserNum = GetLocalUserNumFromPlayerController(PC);

    UE_LOG_ONLINESESSION(Log, TEXT("Received a party match invitation from party leader. Joining the party match."));
